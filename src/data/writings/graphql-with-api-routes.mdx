...
This gets interesting when you're using a GraphQL client like Apollo or Urql, where hooks like `useQuery(){:ts}` will attempt to fetch client-side by default whenever their cache is empty or stale. The question becomes: how do you do this without exposing private API keys to the browser?
